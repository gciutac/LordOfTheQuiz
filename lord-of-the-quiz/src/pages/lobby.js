...
import { Container, Table, Button, Card, FloatingLabel, Row, Col } from 'react-bootstrap'
  ...
  useEffect(() => {
    ...
    if (gameId) {
      fetch(`/api/create-game/${gameId}`)
        ...
        .then((data) => updateGame({ gameId: quizData.gameId, gameKey: game.gameKey}))
          ...
          <tbody>
            <tr>
              <td>
                ...
                <Card.Text className="text-center">Game Code: {game.gameKey}</Card.Text>
                <Card.Img variant="top" src="/FlvdvsTq_400x400.jpg" style={{ width: '10rem' }} />
                <Card.Body>
